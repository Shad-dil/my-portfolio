...
import LanguageSwitcher from "./LanguageSwitcher"; // ← ADD
...
export default function Navbar() {
  ...
  const menuItems = ["about", "projects", "experience", "contact"] as const;
      ...
      <nav className="hidden md:flex items-center gap-8 text-sm">
        {menuItems.map((item) => (
          ...
          >
            <a href={`#${item}`}>
              {t(item)}{" "}
              {/* ← was: item.charAt(0).toUpperCase() + item.slice(1) */}
            </a>
            {active === item && (
              <motion.div
